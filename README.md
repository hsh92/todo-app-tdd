...
## 📋 초기 요구사항 (Requirements)

본 프로젝트는 아래의 요구사항 프롬프트를 기반으로 TDD 방식으로 개발되었습니다.

### 1. 기능 명세
- `addTodo` : 새 할 일 추가
- `getTodo` : 전체 목록 추가 (전체 목록 조회 및 정렬로 기능 구체화)
- `toggleTodo` : 완료 상태 토글
- `deleteTodo` : 할 일 삭제
- `updateTodo` : 할 일 수정
...
### 3. 테스트 원칙
- 각 함수당 최소 3개의 테스트 케이스 작성
- 정상 케이스 및 예외/엣지 케이스 포함
- `describe`/`it` 구조 사용
- 한글 테스트 설명 적용

### 4. 기술 스펙
- Next.js 최신 버전 사용 (`npx create-next-app@latest .` 실행)
- Shadcn UI 사용
...
초기 요구사항 프롬프트를 구현하는 과정에서 다음과 같은 아키텍처 개선 및 명세 수정이 반영되었습니다.

1. **`getTodo` 기능의 합리적 수정**:
   - 요구사항 프롬프트 상의 `getTodo : 전체 목록 추가`는 문맥상 오타로 판단하여 **"전체 목록 조회 및 정렬"** 기능으로 재설계했습니다.
   - 엣지 케이스로 `createdAt` 기준 내림차순(최신순) 정렬 기능을 추가하여 대시보드에서 최신 할 일이 항상 위에 노출되도록 구현했습니다.
2. **순수 함수 아키텍처 도입**:
   - 상태 변화를 추적하기 쉽고 유닛 테스트가 외부 환경에 오염되지 않도록, 핵심 CRUD 로직 함수들을 입력 배열을 받아 새로운 배열을 반환하는 **순수 함수(Pure Functions)**로 설계했습니다.
3. **입력 검증 강화 (엣지 케이스)**:
   - 할 일 등록(`addTodo`) 및 수정(`updateTodo`) 시 제목이 빈 문자열이거나 공백만 있는 경우 예외를 던지도록 설계했습니다.
   - 할 일 등록(`addTodo`) 시 제목이 100자를 초과하는 경우에 대한 예외 처리를 추가하여 데이터 오염을 예방했습니다.
4. **글로벌 ID 생성 헬퍼**:
   - ID 생성 시 브라우저 환경의 `crypto.randomUUID()`를 우선 사용하고, 테스트 환경 등 미지원 환경에서는 fallback 알고리즘을 사용하도록 설계했습니다.
...
PASS src/lib/todo.test.ts
  Todo TDD 핵심 기능 테스트
    addTodo - 새 할 일 추가
      ...
      √ 빈 문자열이나 공백만 있는 제목을 입력하면 예외(Error)를 발생시킨다. (20 ms)
      √ 제목이 100자를 초과하는 경우 예외(Error)를 발생시킨다. (1 ms)
    getTodo - 전체 목록 조회 및 정렬
      √ 전체 목록을 조회하면 저장된 모든 할 일 목록을 반환한다.
      √ 빈 목록일 경우 빈 배열을 반환한다.
      √ 생성일자(createdAt) 기준 내림차순(최신순)으로 정렬하여 반환한다.
    toggleTodo - 완료 상태 토글
      √ 특정 ID를 제공하면 해당 할 일의 완료 상태(completed)를 토글한다.
      √ 존재하지 않는 ID를 토글하려고 하면 원본 목록을 그대로 반환한다. (1 ms)
      √ 빈 목록에서 토글을 시도하면 빈 배열을 그대로 반환한다.
    deleteTodo - 할 일 삭제
      √ 특정 ID를 제공하면 해당 할 일을 목록에서 삭제한다. (1 ms)
      √ 존재하지 않는 ID를 삭제하려고 하면 원본 목록을 그대로 반환한다.
      √ 여러 개의 할 일 중 정확히 지정된 ID의 항목만 삭제한다. (1 ms)
    updateTodo - 할 일 수정
      √ 특정 ID와 새 제목을 제공하면 해당 할 일의 제목을 수정한다.
